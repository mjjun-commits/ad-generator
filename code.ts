...
interface Variant {
  id: string;
  template: string;
  texts?: Record<string, string>;        // { "main-text": "...", "cta-text": "..." }
  bg_color?: string;
  visible_thumbs?: string[];             // ["thumb1", "thumb3"] — 보여줄 thumb 목록
  images?: Record<string, string>;       // { "thumb1": "photo1.jpg" } — 레이어명: 파일명
}
...
function findTextNode(node: SceneNode, name: string, depth = 0): TextNode | null {
...
async function setTextSafely(textNode: TextNode, text: string): Promise<void> {
...
// 메인 실행
figma.ui.onmessage = async (msg: { type: string; data?: VariantsJSON; images?: Record<string, number[]> }) => {
  if (msg.type === 'cancel') {
    figma.closePlugin();
    return;
  }

  if (msg.type !== 'run' || !msg.data) return;
  ...
  if (msg.data.campaign === 'debug') {
    ...
    for (const page of figma.root.children) {
      ...
      const frames = (page as PageNode).children
  ...
  for (let i = 0; i < variants.length; i += CHUNK_SIZE) {
    ...
    for (const variant of chunk) {
      try {
        ...
        let templateFrame: FrameNode | undefined;
        for (const page of figma.root.children) {
          if (page.name === 'OUTPUT') continue;
          await (page as PageNode).loadAsync();
          const found = (page as PageNode).findOne(
            n => n.name === variant.template && n.type === 'FRAME'
          ) as FrameNode | undefined;
          if (found) { templateFrame = found; break; }
        }
        ...
        const GAP = 40;
        const COLS = 5;
        const outputFrames = outputPage.children.filter(n => n.type === 'FRAME') as FrameNode[];
        ...
        if (variant.bg_color) {
          ...
          // 프레임 자체 배경색
          if (cloned.fills && (cloned.fills as Paint[]).length > 0) {
          ...
          if (bgNode && 'fills' in bgNode) {
            try {
              if ('setFillStyleIdAsync' in bgNode) {
                await (bgNode as MinimalFillsMixin).setFillStyleIdAsync('');
              }
            } catch (_) { /* 스타일 없으면 무시 */ }
            (bgNode as GeometryMixin).fills = [solidFill];
          }
        }
        ...
        if (variant.visible_thumbs) {
          ...
          if (group33 && 'children' in group33) {
            for (const child of (group33 as GroupNode).children) {
        ...
        if (variant.images && msg.images) {
          for (const [layerName, filename] of Object.entries(variant.images)) {
            ...
            if (!targetNode || !('fills' in targetNode)) continue;
            ...
            (targetNode as GeometryMixin).fills = [imageFill];
          }
        }
